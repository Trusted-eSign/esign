...
then
    ...
    until ! ${CUR_DIR}/certutil/osx/certutil -D -n "eSign Local CA" -d sql:"${firefoxDefaultProfile}"
    do
        echo "eSign certificate was removed from sql:cert.db"
        #echo "eSign certificate was removed from sql:cert.db" >> $FILE
    done
    until ! ${CUR_DIR}/certutil/osx/certutil -D -n "eSign Local CA" -d dbm:"${firefoxDefaultProfile}"
    do
        echo "eSign certificate was removed from dbm:cert.db"
        #echo "eSign certificate was removed from dbm:cert.db" >> $FILE
